...
onready var text = get_parent().get_node("dialogue_before_mike").before_mike
...
func _physics_process(delta):
	...
	if $MikePlayer/TextBox2/Label.text == "Mike":
		if alignment == "Good":
			...
			$BcPlayer.visible = false
			$BcEnemy.visible = false
	if $MikePlayer/TextBox2/Label.text == "BC":
		$MikePlayer.visible = false
		$MikeEnemy.visible = false
		if alignment == "Good":
...
func _on_Button1_pressed():
	if $Button1.text == "Yes":
		get_tree().change_scene("res://src/battle_bc_mike.tscn")

func _on_Button2_pressed():
	get_tree().change_scene("res://src/battle_sf_mike.tscn")
